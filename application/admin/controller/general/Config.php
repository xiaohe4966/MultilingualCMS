...
class Config extends Backend
{
    ...
    public function index()
    {
        ...
        if(!$isSuperadmin){
            //分割成数组
            $config_group_show = explode('|', \think\Config::get('site.config_group_show'));
            $config_field_hide = explode('|', \think\Config::get('site.config_field_hide'));
        }
        foreach ($groupList as $k => $v) {
            if(!$isSuperadmin){
                if(!in_array(strtolower($k), $config_group_show)){
                    continue;
                }
            }
            $siteList[$k]['name'] = $k;
        ...
        foreach ($this->model->all() as $k => $v) {
            ...
            if(!$isSuperadmin){
                //字段找到就隐藏
                if(in_array($value['name'], $config_field_hide)){
                    continue;
                }
            }
        ...
        $GroupList = ConfigModel::getGroupList();
        if(!$isSuperadmin){
            unset($GroupList['superadmin']);
            unset($siteList['superadmin']);
        }
