...
class Crud extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $multieditFile = $viewDir . 'multiedit.html';
        ...
        if ($delete) {
            $readyFiles = [$controllerFile, $modelFile, $validateFile, $addFile, $editFile, $indexFile, $multieditFile, $recyclebinFile, $langFile, $javascriptFile];
        ...
        try {
            ...
            if(Config::get('site.crud_copy_switch')) {
                $operateButtonList[] = "{name: 'copy',title: __('Copy'),text: __('Copy'),classname: 'btn btn-xs btn-info btn-ajax',icon: 'fa fa-copy',url: function(row){return '" . strtolower($controllerName) . "/copy?ids=' + row.id}}";
            }
            ...
            if(in_array('cate_id', $fieldArr))
                $this->writeToFile('multiedit', $data, $multieditFile);
            if ($recyclebinHtml) {
...
EOD;
    ...
    protected function getJsColumn($field, $datatype = '', $extend = '', $itemArr = [], $fieldConfig = [])
    {
        ...
        if ($field == 'cate_id') {
            $html .= ", events: Table.api.events.search";
        }
