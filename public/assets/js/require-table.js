    ...
    var Table = {
        ...
        api: {
            ...
            bindevent: function (table) {
                ...
                // 批量编辑按钮事件
                toolbar.on('click', Table.config.movecate, function () {
                    var ids = Table.api.selectedids(table);
                    var url = options.extend.movecate_url;
                    if (ids.length < 1) {
                        Layer.alert('请选择要操作的数据');
                        return false;
                    }
                    url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    Fast.api.open(url, $(this).data("original-title") || $(this).attr("title") || __('movecate'), {});
                });
                ...
                // 批量编辑按钮事件
                toolbar.on('click', Table.config.multieditbtn, function () {
                    var ids = Table.api.selectedids(table);
                    var url = options.extend.multiedit_url;
                    if (ids.length < 1) {
                        Layer.alert('请选择要操作的数据');
                        return false;
                    }
                    url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    Fast.api.open(url, $(this).data("original-title") || $(this).attr("title") || __('multiedit'), {});
                });
